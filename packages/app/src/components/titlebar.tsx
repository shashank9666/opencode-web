...
export function Titlebar(props: { update?: TitlebarUpdate }) {
  ...
  const hasProjects = createMemo(() => (layout.projects.list()?.length ?? 0) > 0)
      ...
      <Switch>
        <Match when={useV2Titlebar()}>
          {(_) => {
            ...
            const ideHref = () => {
              if (params.dir) return `/${params.dir}/ide`

              const project = layout.projects.list()?.[0]
              if (!project) return "/"

              return `/${base64Encode(project.worktree)}/ide`
            }
            ...
            const activeDir = () => {
              ...
              return base64Encode(project.worktree)
            }

            const activeSessionId = createMemo(() => {
              const dirVal = activeDir()
              if (!dirVal) return undefined
              const decoded = decode64(dirVal)
              if (!decoded) return undefined
              const [dirStore] = serverSync().child(decoded, { bootstrap: true })
              const sessions = sortedRootSessions(dirStore, Date.now())
            ...
            return (
              ...
              >
                ...
                />
                 ...
                 <Show when={activeDir()}>
                  {(dir) => (
                    <div class="flex items-center bg-[var(--v2-background-bg-layer-02)] p-0.5 rounded-md gap-0.5 shrink-0 [app-region:no-drag] select-none h-8">
                      <button
                        type="button"
                        class="px-2.5 py-1 text-12-medium rounded-md transition-colors font-medium h-7 flex items-center"
                        classList={{
                          "bg-[var(--v2-background-bg-layer-04)] text-[var(--v2-text-text-base)] shadow-sm": !location.pathname.includes("/ide"),
                          "text-[var(--v2-text-text-muted)] hover:text-[var(--v2-text-text-base)]": location.pathname.includes("/ide"),
                        }}
                        onClick={() => {
