...
export function createPromptSubmit(input: PromptSubmitInput) {
  ...
  const abort = async () => {
    const sessionID = input.info()?.id || input.sessionId?.() || params.id
  ...
  const handleSubmit = async (event: Event) => {
    ...
    const currentSessionId = input.info()?.id || input.sessionId?.() || params.id
    ...
    if (!session && !isNewSession) {
      session = { id: currentSessionId! } as any
    }
    if (!session && isNewSession) {
