...
function createWorkspaceTerminalSession(
  ...
  return {
    ...
    newShell(options: { command?: string; args?: string[]; title?: string }) {
      const nextNumber = pickNextTerminalNumber()
      const title = options.title ?? defaultTitle(nextNumber)

      sdk.client.pty
        .create({ command: options.command, args: options.args, title })
        .then((pty: { data?: { id?: string; title?: string } }) => {
          const id = pty.data?.id
          if (!id) return
          const newTerminal = {
...
export const { use: useTerminal, provider: TerminalProvider } = createSimpleContext({
  ...
  init: () => {
    ...
    return {
      ...
      new: () => workspace()?.new(),
      newShell: (options: { command?: string; args?: string[]; title?: string }) => workspace()?.newShell(options),
