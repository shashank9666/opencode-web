...
export const { use: useCommand, provider: CommandProvider } = createSimpleContext({
  ...
  init: () => {
    ...
    const [store, setStore] = createStore({ suspendCount: 0 })
    // Plain JS array — never stored in a signal or store so Solid's proxy
    // system never touches it and function references stay intact.
    type RegEntry = { key: string | undefined; getOptions: () => CommandOption[] }
    const registrationList: RegEntry[] = []
    // Counter signal used only to notify Solid of structural changes (add/remove).
    const [registrationTick, setRegistrationTick] = createSignal(0, { equals: false })
    ...
    const registered = createMemo(() => {
      registrationTick() // track structural changes
      const seen = new Set<string>()
      const all: CommandOption[] = []

      for (const reg of registrationList) {
        const options = reg.getOptions?.()
    ...
    function register(key: string | (() => CommandOption[]), cb?: () => CommandOption[]) {
      ...
      if (!next) return
      const entry: RegEntry = { key: id, getOptions: createMemo(next) }
      if (id === undefined) {
        registrationList.unshift(entry)
      } else {
        const idx = registrationList.findIndex((x) => x.key === id)
        if (idx >= 0) registrationList.splice(idx, 1)
        registrationList.unshift(entry)
      }
      setRegistrationTick((t) => t + 1)
      onCleanup(() => {
        const idx = registrationList.indexOf(entry)
        if (idx >= 0) registrationList.splice(idx, 1)
        setRegistrationTick((t) => t + 1)
      })
    }
    ...
    return {
      ...
      keybinds(enabled: boolean) {
        setStore("suspendCount", (count: number) => Math.max(0, count + (enabled ? -1 : 1)))
      },
      suspended,
      get catalog() {
